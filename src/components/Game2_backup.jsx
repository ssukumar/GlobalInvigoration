    ...
    if (gamePhase === 'reaching') {
      ...
      if (coinVisible) {
        ...
        if (keySequence.length > 0) {
          ...
          const keySpacing = 60; // fixed spacing regardless of sequence length
          const totalWidth = (keySequence.length - 1) * keySpacing;
          const startX = canvasSize.width / 2 - totalWidth / 2;
          
           let sequenceY;
          if (currentRewardValue === 10) {
            sequenceY = coinPosition.y + pileHeight + 160;
         } else {
              sequenceY = coinPosition.y + pileHeight + 140;
      }
          ...
          keySequence.forEach((key, index) => {
            ...
            ctx.fillText(key.toUpperCase(), startX + index * keySpacing, sequenceY);
          });
