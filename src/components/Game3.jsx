...
const Game3 = ({ participantData, participantId, onGameComplete }) => {
  ...
  const numBlocks = useRef(GAME_CONFIG.BLOCKS.ORDER.length);
  ...
  const handleKeyPress = (event) => {
    ...
    if (isValid) {
      ...
      if (nextIndex < keySequence.length) {
        ...
        if (nextRound > GAME_CONFIG.BLOCKS.ORDER[currentBlock - 1].baseRounds) {
          // Completed current block, check if more blocks remain
          if (currentBlock < numBlocks.current) {
            // Move to next block
            setCurrentBlock(prev => prev + 1);
            setGamePhase('break');
          } else {
            // All blocks completed, end game
            setGameActive(false);
        ...
        //   setCurrentRewardValue(getRewardSequence(currentEnvironment)[(nextRound - 1) % getRewardSequence(currentEnvironment).length]);
        }

        // if (currentEnvironment === 'poor' && nextRound > GAME_CONFIG.BLOCKS.POOR.baseRounds - 1) {
        //   // Switch to break
        //   setGamePhase('break');
        // } else if (currentEnvironment === 'rich' && nextRound > GAME_CONFIG.BLOCKS.RICH.baseRounds + 1) {
        //   // Game completed
        //   setGameActive(false);
        //   gameCompletedRef.current = true;
        // } else {
